...
func NewExporter(target string, port int, period time.Duration, timeout time.Duration) *Exporter {
	return &Exporter{
		...
		sentBitsPerSecond:     prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "sent_bits_per_second"), "Total sent bits per second.", nil, nil),
		...
		receivedBitsPerSecond: prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "received_bits_per_second"), "Total received bits per second.", nil, nil),
	}
}
